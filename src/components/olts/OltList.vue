...
onMounted(async () => {
  ...
  const HIDE_PHONE = Array.from({ length: totalCols }, (_, i) => i).filter(
    (i) => !KEEP_PHONE.includes(i)
  );

  // TABLET (576–1366.98): tampil 4 kolom pertama + kolom terakhir
  const KEEP_TABLET = [0, 1, 2, 3, LAST]; // bukan [0,1,2,3,4,LAST]
  const HIDE_TABLET = Array.from({ length: totalCols }, (_, i) => i).filter(
    (i) => !KEEP_TABLET.includes(i)
  );
  ...
  dataTableInstance = $("#datatable-default").DataTable({
    ...
    headerCallback: function () {
      const api = this.api();
      HIDE_PHONE.forEach((i) =>
        $(api.column(i).header()).addClass("hide-phone")
      );
      HIDE_TABLET.forEach((i) =>
        $(api.column(i).header()).addClass("hide-tablet")
      );
    },
  });
